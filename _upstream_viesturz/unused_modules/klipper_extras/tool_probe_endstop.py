...
# The active tool endstop can be switched either via SET_ACTIVE_TOOL_PROBE TOOL=99
# or via auto-detection of a single open tool probe using DETECT_ACTIVE_TOOL_PROBE.
class ToolProbeEndstop:
    def __init__(self, config):
        ...
        self.tool_probes = {}
        self.last_query = {}  # map: tool number -> endstop state
        ...
        # Emulate the [probe] object, since other modules rely on it.
...
>>>>>>> a85befe6eab3ab4da6a1de283d80b69f64b8dee7
    ...
    cmd_SET_ACTIVE_TOOL_PROBE_help = "Set which tool probe acts as the Z endstop."
    ...
    cmd_DETECT_ACTIVE_TOOL_PROBE_help = "Detect the active tool by finding exactly one probe that is NOT triggered."
    ...
    cmd_START_TOOL_PROBE_CRASH_DETECTION_help = "Start detecting probe crashes for the current tool."
    def cmd_START_TOOL_PROBE_CRASH_DETECTION(self, gcmd):
        # Detect waits until previous print moves are finished to query triggers
    ...
    cmd_STOP_TOOL_PROBE_CRASH_DETECTION_help = "Stop detecting probe crashes."
    def cmd_STOP_TOOL_PROBE_CRASH_DETECTION(self, gcmd):
<<<<<<< HEAD
        # Clear after the current print queue is finished
...
# Routes commands to the currently selected tool-probe endstop.
class EndstopRouter:
    ...
    def get_position_endstop(self):
        if not self.active_mcu:
            # This will be picked up by the endstop and is static.
            # Report 0 and fix up during the homing sequence.
            return 0.0
